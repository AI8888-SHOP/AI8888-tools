...
type WorkspaceCenterProps = {
  section?: "workspace" | "usage" | "settings";
...
export default function WorkspaceCenter({ section = "workspace", profiles, selectedProfileId, onProfileApplied }: WorkspaceCenterProps) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setMessage(section === "workspace"
        ? `已载入 ${data.mcpServers.length} 个 MCP、${data.prompts.length} 个 Prompt、${data.skills.length} 个 Skill`
        : section === "usage"
          ? "统计来自本地路由请求日志"
          : "代理、备份与诊断设置已载入");
      setError("");
  ...
  useEffect(() => { setTab(section === "workspace" ? "projects" : section === "usage" ? "usage" : "proxy"); }, [section]);
  ...
  const tabs = section === "workspace"
    ? [["projects", "项目"], ["mcp", "MCP"], ["prompts", "Prompts"], ["skills", "Skills"]]
    : section === "usage"
      ? [["usage", "本地路由用量"]]
      : [["proxy", "代理容错"], ["backup", "备份与诊断"]];
  const sectionTitle = section === "workspace" ? "工作区资源" : section === "usage" ? "本地路由用量" : "维护与高级设置";
  const sectionBadge = section === "workspace" ? `${workspace.projects.length} 个项目` : section === "usage" ? "最近 30 天" : "本地设置";

  return <section className="panel workspaceCenter">
    <div className="panelHead"><div><h2>{sectionTitle}</h2><p className="muted workspaceSummary">{section === "workspace" && activeProject ? `当前项目：${activeProject.name}` : message}</p></div><span className="badge">{sectionBadge}</span></div>
    {error && <div className="inlineAlert">{error}</div>}
    {tabs.length > 1 && <div className="segmented workspaceTabs" role="tablist">{tabs.map(([id, label]) => <button className={tab === id ? "active" : ""} aria-selected={tab === id} onClick={() => setTab(id)} key={id} type="button" role="tab">{label}</button>)}</div>}
    ...
    {tab === "mcp" && <div className="workspacePane">
      ...
      <details className="workspaceComposer" open={mcpComposerOpen} onToggle={(event) => setMcpComposerOpen(event.currentTarget.open)}>
        <summary>{mcp.id ? `编辑 MCP：${mcp.name || mcp.id}` : "新增 MCP"}</summary>
    ...
    {tab === "usage" && <div className="workspacePane">
      <div className="actions workspaceToolbar"><button onClick={() => void refreshUsage()} disabled={busy}>刷新 30 天统计</button><button className="danger" onClick={() => void clearUsage()} disabled={busy}>清空本地日志</button></div>
      {usage ? <><div className="metrics usageMetrics"><div><small>请求</small><strong>{count(usage.totalRequests)}</strong></div><div><small>成功率</small><strong>{usage.totalRequests ? `${((Number(usage.successfulRequests || 0) / usage.totalRequests) * 100).toFixed(1)}%` : "-"}</strong></div><div><small>Token</small><strong>{count(Number(usage.inputTokens || 0) + Number(usage.outputTokens || 0))}</strong></div><div><small>估算成本</small><strong>{money(usage.totalCostUsd)}</strong></div></div><div className="list workspaceList">{usage.byModel?.map((item) => <article className="row" key={item.model}><div><strong>{item.model}</strong><small>{count(item.requests)} 次 · 输入 {count(item.inputTokens)} · 输出 {count(item.outputTokens)}</small></div><span>{money(item.costUsd)}</span></article>)}</div></> : <div className="emptyState compactEmpty"><strong>尚未载入本地路由用量</strong><p className="muted">刷新后显示请求、Token 和按模型估算的成本。</p></div>}
      <details className="workspaceComposer"><summary>模型价格设置</summary><div className="workspaceForm priceForm"><label>模型<input value={price.model} onChange={(event) => setPrice({ ...price, model: event.target.value })} /></label><label>输入 / 百万 Token<input type="number" value={price.inputPerMillion} onChange={(event) => setPrice({ ...price, inputPerMillion: Number(event.target.value) })} /></label><label>输出 / 百万 Token<input type="number" value={price.outputPerMillion} onChange={(event) => setPrice({ ...price, outputPerMillion: Number(event.target.value) })} /></label><label>缓存输入 / 百万 Token<input type="number" value={price.cachedInputPerMillion} onChange={(event) => setPrice({ ...price, cachedInputPerMillion: Number(event.target.value) })} /></label><button onClick={() => void savePrice()} disabled={busy || !price.model}>保存价格</button></div></details>
    </div>}
